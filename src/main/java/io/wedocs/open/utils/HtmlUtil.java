...
public class HtmlUtil {
    ...
    public static void fixImageSourceUrls(Map<String, Object> fileContents, JBakeConfiguration configuration) {
        String htmlContent = fileContents.get(Crawler.Attributes.BODY).toString();
        ...
        fileContents.put(Crawler.Attributes.BODY, document.body().html());
    }

    private static String getDocumentUri(Map<String, Object> fileContents) {
        String uri = fileContents.get(Crawler.Attributes.URI).toString();

        if (fileContents.get(Crawler.Attributes.NO_EXTENSION_URI) != null) {
            uri = fileContents.get(Crawler.Attributes.NO_EXTENSION_URI).toString();
